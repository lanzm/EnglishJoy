...
摒弃任何社交、积分、任务、PK 等繁杂花哨的干扰，回归记忆卡片（Flashcards）最纯粹的本源，为英语学习者提供一个绝对专注、秒级启动、完全离线的英语记忆空间。
...
- **本地词库集**：内置 **CET4、CET6、考研、雅思** 四本学术核心词书，本地数据一次性载入，断网不断学。
- **无网释义解析**：释义列表、构词法剖析、中英例句对比等核心逻辑完全在本地沙箱中执行，秒级响应。

### 📱 拟真层叠卡片交互 (Flashcard Interaction)
- **3D 叠层视差**：采用层叠 DOM 卡片结构，底层卡片与前景卡片形成视差深度感。
- **手势阻尼跟手**：精心设计的物理阻尼拖拽算法，支持垂直拖拽（向上划过进入下一词，向下拖拽退回上一词温习），未达阈值平滑回弹。
